...
extern void JEM_FFICallFn(void *fnRef, void *argStack, 
...
jint JEM_CallForeignFunction(JNIEnv *env, JEMCC_Object *thisObj,
    ...
    jint index, allocSize = 0, argSize, retType = 0;
    ...
    /* Build the return type (and validate return argument space) */
    if (fnDesc->method_info.returnDescriptor == NULL) {
        ...
        switch (fnDesc->method_info.returnDescriptor->generic.tag) {
            ...
            case BASETYPE_Short:
            case BASETYPE_Boolean:
                /* These are all the same in IA-32 function returns */
                allocSize = 4;
    ...
    if (allocSize < argSize) allocSize = argSize;

    /* Allocate a temporary array on the local stack */
    if (allocSize != 0) {
        argStack = alloca(allocSize);
        if (argStack == NULL) {
    ...
    *((JNIEnv **) ptr) = env;
        ...
        switch (descPtr->generic.tag) {
            ...
            case BASETYPE_Boolean:
                *((jint *) ptr) = argList[index].intVal;
                ...
                break;
            case BASETYPE_Long:
                *((jlong *) ptr) = argList[index].longVal;
                ptr += 8;
                break;
            case BASETYPE_Float:
                *((jfloat *) ptr) = argList[index].fltVal;
                ...
                break;
            case BASETYPE_Double:
                *((jdouble *) ptr) = argList[index].dblVal;
                ptr += 8;
                break;
            default:
                abort();
    ...
    JEM_FFICallFn(fnRef, argStack, argSize, retType);
